"""MVTec AD evaluation across the full 15-category benchmark.
...
default (our strongest frozen extractor). Router, privacy filter, and outbox are unchanged.
    ...
    python -m eval.run_ad --data /scratch/sss6371/datasets/mvtec_ad --backbone dinov2 --real-cloud
"""
...
def _category(data, category, backbone):
    ...
    train = [p for p, _l, _c in load_mvtec(data, category, "train")]
    test = list(load_mvtec(data, category, "test"))
...
def main() -> None:
    ...
    parser.add_argument("--categories", nargs="+", default=AD_CATEGORIES)
    parser.add_argument(
        "--backbone", default="dinov2", choices=["handcrafted", "mobilenet", "dinov2"]
    )
    parser.add_argument("--real-cloud", action="store_true")
    parser.add_argument("--out", default="eval/results_ad.md")
