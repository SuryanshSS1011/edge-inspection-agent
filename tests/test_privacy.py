...
def test_unlisted_context_keys_are_dropped_silently():
    pf = PrivacyFilter(mode="embedding")
    payload = pf.filter(_frame(), bbox=(8, 8, 32, 32), context={"random_note": "hi"})
    assert "random_note" not in payload.context
    # Not allowed and not explicitly PII => not sent, not logged as a crossing.
    assert all("random_note" not in c.field for c in payload.crossings)
